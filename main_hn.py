...
def main(save_fg_mask=False, img_size=224, output_folder="outputs"):
    ...
    patch_h = patch_w = img_size // 14 # each patch 14 pixel, patch_h and patch_w are number of patches along width and height
    # for this image, there are 32 x 32 = 1024 patches /image
    # each patch has embedding vector of 384
    start_idx = 1
    end_idx = 4
    ...
    with torch.no_grad():
        ...
        #x_norm_patchtokens.shape = (4, 1024, 384), ie 4 image, each image 1024 patches, each patch has an embedding of 384 numbers

    x_norm_1616_patches = x_norm_patchtokens.reshape(img_cnt*patch_h*patch_w, -1)
    # x_norm_1616_patches.shape = (4096, 384), 4096 is the total number of patches of 4 images (1024 x 4)

    fg_pca = PCA(n_components=1)
    fg_pca_images = fg_pca.fit_transform(x_norm_1616_patches)
    # fg_pca_images.shape = (4096, 1), since just take 1st component of PCA, 
    fg_pca_images = minmax_scale(fg_pca_images)
    fg_pca_images = fg_pca_images.reshape(img_cnt, patch_h*patch_w)
    # fg_pca_images.shape = (4, 1024), ie 4 images, each image has 1024 numbers for 1024 patches
    ...
    if save_fg_mask:
        for i in range(img_cnt):
            ...
            image_patches[np.logical_not(mask)] = 0 # if mask is False (< than threshold), not of it is True, the coresponding patch = 0
            image_patches = image_patches.reshape([patch_h, patch_w])
            # image_patches.shape = (32, 32)
    ...
    # i.e. here we are selecting foreground patches, totaly we have 1630 foreground patches from 4 images
    pca_features = pca.fit_transform(fg_patches) # reduce from 384 to 3 dimensions
    fg_result = minmax_scale(pca_features)
    #fg_result.shape = (1630, 3), i.e. dimension of 1630 foreground patches has been reduced to 3
